...
class Generator(nn.Module):
    def __init__(self, latent_dim, task_cond_dim, output_dim):
        ...
        self.model = nn.Sequential(
            *block(latent_dim + task_cond_dim, 256, normalize=False),
            *block(256, 512),
            *block(512, 1024),
            *block(1024, 2048),
    ...
    def forward(self, z, task_cond):
        z = torch.cat([z, task_cond], dim=1)
        obs = self.model(z)
        return obs
    
    def sample(self, task_cond): # sample a batch of observations with task conditioning added
        batch_size = task_cond.shape[0]
        z = torch.randn(batch_size, self.latent_dim).cuda()
        obs = self.forward(z, task_cond)
        return torch.cat([obs, task_cond], dim=1)

class Discriminator(nn.Module):
    def __init__(self, input_dim, task_cond_dim):
        super(Discriminator, self).__init__()

        self.model = nn.Sequential(
            nn.Linear(input_dim + task_cond_dim, 512),
    ...
    def forward(self, obs, task_cond):
        x = torch.cat([obs, task_cond], dim=1)
...
class Trainer(object):
    def __init__(self, generator, discriminator, dataset, train_batch_size = 64, n_critic = 5, ckpts_folder = './ckpts', num_workers = 4):
        ...
        self.dl = DataLoader(self.dataset, batch_size = train_batch_size, shuffle=True, pin_memory=True, num_workers=num_workers, drop_last=True)
    ...
    def load_new_dataset(self, dataset):
        self.dataset = dataset
        self.dl = DataLoader(self.dataset, batch_size = self.batch_size, shuffle=True, pin_memory=True, num_workers=self.num_workers, drop_last=True)

    def _compute_gradient_penalty(self, real_samples, fake_samples, task_cond):
        ...
        d_interpolates = self.discriminator(interpolates, task_cond)
    ...
    def train(self, num_epoch):
        ...
        for _ in range(num_epoch):
            ...
            for i, real_obs in enumerate(self.dl):
                real_obs = real_obs.cuda()
                ...
                assert real_obs.shape[0] > 1
                assert task_cond.shape[0] > 1
                assert z.shape[0] > 1
                fake_obs = self.generator(z, task_cond)

                # Real images
                real_validity = self.discriminator(real_obs, task_cond)
                # Fake images
                fake_validity = self.discriminator(fake_obs, task_cond)
                # Gradient penalty
                gradient_penalty = self._compute_gradient_penalty(real_obs, fake_obs, task_cond)
                # Adversarial loss
