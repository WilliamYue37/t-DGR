...
for repeat in range(repeats):
    for env_name in env_names:
        ...
        if learner_trainer is not None and generator_trainer is not None:
            ...
            num_generated_trajs = math.ceil(num_generated_samples / args.batch_size / generator_model.horizon)
            ...
            for j in range(max(num_generated_trajs, num_of_env_so_far)):
                task_id = j % num_of_env_so_far
                task_cond = torch.eye(len(env_names))[task_id].cuda()

                traj_data = prev_generator.sample(task_cond=task_cond, batch_size=args.batch_size)
                ...
                with torch.no_grad():
                    assert image_data.shape[0] % args.batch_size == 0
                    idx = 0
                    while idx < image_data.shape[0]:
                        data = image_data[idx:idx+args.batch_size].cuda()
